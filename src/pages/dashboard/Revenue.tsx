...
const Revenue = () => {
  ...
  const fetchRevenueStats = async () => {
    try {
      ...
      const {
        ...
        error: ordersError
      } = await supabase.from("orders").select("total_amount, delivery_fee, platform_fee, gst_charges, items").eq("status", "delivered");
      ...
      } = await supabase.from("seller_wallet_transactions").select("amount").eq("type", "withdrawal").not("description", "like", "%Pending%");
      ...
      const {
        data: refundedOrders,
        error: refundsError
      } = await supabase.from("orders").select("total_amount").eq("status", "refunded");
  ...
  return <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map(stat => <Card key={stat.title} className="border-border">
      ...
      <Card className="border-border">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {breakdownCards.map(item => <div key={item.title} className="p-4 bg-muted/50 rounded-lg">
